...
export function CharacterAvatar({
  ...
  // 空状态：显示问号或叉号（头像框带边框，问号本身无边框）
  if (isEmpty || !charId || imgError) {
    return (
      <div className={`relative ${sizeClass} ${className}`}>
        ...
        >
          {emptyType === 'unknown' ? (
            <span className="text-fg-2 font-bold" style={{ fontSize: size === 'sm' ? '16px' : size === 'md' ? '20px' : '24px' }}>
              ?
            </span>
          ) : (
  ...
  // 显示角色头像
  const imagePath = `/charimg/${charId}.png`;
  ...
  return (
    <div className={`relative ${sizeClass} ${className}`}>
      {/* 头像容器 */}
      <div 
        className={`w-full h-full rounded-full border-2 ${borderClass} overflow-hidden bg-bg-3 relative`}
      >
        {/* 加载中占位 */}
        {!imgLoaded && (
          <div className="absolute inset-0 bg-bg-3 animate-pulse" />
        )}
